...
class UnifyingAutoEncoder:
    def __init__(self, inp_shapes,
                 unifying_dim=100, num_unifying_layers=1,
    ...
    def _init_model(self):
        ...
        unifying_strata_after_ae = [
            ELU()(stratum)
            for stratum in unifying_strata_after_ae
        ]

        # We need to expand the dimensions to those that we had on input
        output_strata = [
            Dense(out_shape, kernel_regularizer=l2(self.reg))(stratum)
            for stratum, out_shape in zip(unifying_strata_after_ae, self.inp_shapes)
        ]
